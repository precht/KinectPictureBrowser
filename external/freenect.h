...
public:
    ...
    void setVideoFormat(freenect_video_format requested_format, freenect_resolution requested_resolution = FREENECT_RESOLUTION_MEDIUM) {
        if (requested_format != m_video_format || requested_resolution != m_video_resolution) {
            ...
            m_rgb_buffer.reset(new uint8_t[static_cast<uint32_t>(mode.bytes)]);
    ...
    freenect_video_format getVideoFormat() const {
        return m_video_format;
    }
    freenect_resolution getVideoResolution() const {
    ...
    freenect_depth_format getDepthFormat() const {
        return m_depth_format;
    }
    freenect_resolution getDepthResolution() const {
...
public:
    Freenect() : m_stop(false) {
        ...
        if(pthread_create(&m_thread, NULL, pthread_callback, static_cast<void*>(this)) != 0)
